// ═══════════════════════════════════════════════════════════
// BELLA'FOOD — Module principal
// Importé dynamiquement par FoodWrapper dans BellaiaApp.tsx
// via : import("../modules/food/FoodF").then(m => m.default)
// ═══════════════════════════════════════════════════════════
import React, { useState } from "react";
import { FOOD_COLORS as FC, FOOD_STOCK_INIT, FOOD_MATERIEL_INIT, FOOD_CONSOMMABLES_INIT } from "./foodConsts";
...
interface Props { user?: { id?: string; prenom?: string; nom?: string; role?: string } }

type Section =
  | "dashboard" | "catalogue" | "recettes" | "commandes"
  | "stocks"    | "materiel"  | "consommables" | "calculateur" | "assistant";
...
export default function FoodF({ user }: Props) {
  ...
  const naviguer = (dest: string) => {
    if (SECTIONS.some(s => s.id === dest)) setSection(dest as Section);
  };

  const titres: Record<Section, string> = {
    dashboard:"Bella'Food", catalogue:"Catalogue", recettes:"Recettes & fiches",
    commandes:"Commandes", stocks:"Stocks matières", materiel:"Matériel & Consomm.",
    consommables:"Consommables", calculateur:"Calculateur de coût", assistant:"Assistant Bellaïa",
  };
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{
        padding:"12px 16px", borderBottom:`1px solid ${FC.line}`,
        display:"flex", justifyContent:"space-between", alignItems:"center",
        background:"rgba(0,0,0,0.4)", flexShrink:0,
      }}>
        <div>
          <div style={{ fontFamily:"serif", fontSize:16, color: FC.or, letterSpacing:2 }}>
            🍃 Bella'Food
          </div>
          ...
          {section !== "dashboard" && (
            <button onClick={() => setSection("dashboard")}
              style={{ background:"none", border:`1px solid ${FC.line}`, borderRadius:8,
                padding:"4px 10px", color: FC.creamD, cursor:"pointer", fontSize:10 }}>
              ‹ Accueil
            </button>
          )}
        </div>
      ...
      {/* Nav principale */}
      <div style={{ display:"flex", gap:0, overflowX:"auto", borderBottom:`1px solid ${FC.line}`,
      ...
      <div style={{ flex:1, overflowY:"auto", padding:16 }}>
        {section === "dashboard" && (
          <FoodDashboard
            commandes={commandes}
            stocks={FOOD_STOCK_INIT}
            consommables={FOOD_CONSOMMABLES_INIT}
            onNavigue={naviguer}
          />
        )}
        ...
        {(section === "materiel" || section === "consommables") && (
          <FoodMateriel onglet={section === "consommables" ? "consommables" : "materiel"} />
        )}

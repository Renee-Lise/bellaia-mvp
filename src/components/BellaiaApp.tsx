...
function PortailClient({ user, produits, evenements, onLogout, onNewCommande }) {
  ...
  if (activeUnivers === "bsh") {
    if (estMineur(user?.date_naissance)) return (
      <div style={{minHeight:"100vh",background:"#0d0b12",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
        ...
        <div style={{fontSize:18,fontWeight:700,color:"#e8dcc8",marginBottom:8,fontFamily:"Georgia,serif"}}>Espace réservé aux personnes majeures</div>
        <div style={{fontSize:13,color:"#9a8fa5",marginBottom:24,maxWidth:300,lineHeight:1.6}}>Cet espace est réservé aux personnes majeures.</div>
    ...
    return <ClientBSH produits={produits} evenements={evenements} onBack={() => setActiveUnivers(null)} onNewCommande={onNewCommande}/>;
  }
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:B.night,fontFamily:SA,color:B.cream}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px 14px 24px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {UNIVERS_CLIENT.filter(u => !(u.id === "bsh" && estMineur(user?.date_naissance))).map(u => (
...
function ClientEvents({ onBack, onNewCommande }) {
  const [cat, setCat] = useState(null);      // catégorie sélectionnée
  const [modal, setModal] = useState(null);  // prestation pour devis/réservation
  ...
  // Demande de devis / réservation via WhatsApp (commande manuelle)
  const demander = (p, type) => {
    const id = "EV" + Date.now().toString().slice(-6);
    if (onNewCommande) onNewCommande({
      id, client:"Client web", produit:`${p.nom} (${type})`,
      montant: p.prix || 0, acompte:0, statut:"Demande reçue",
      date: today(), pmt:"À confirmer", pole:"Events",
      notes:`${type} Events — ${new Date().toLocaleString("fr-FR")}`,
    });
    ...
    return (
      <div style={{display:"flex",flexDirection:"column",height:"100vh",background:`radial-gradient(ellipse at 20% 0%,${EV.night},#070d0a 65%)`,fontFamily:SA,color:EV.creme}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:16,display:"flex",flexDirection:"column",gap:12}}>
          ...
          {prestas.map(p => (
            <div key={p.id} style={{background:EV.verre,border:`1px solid ${EV.line}`,borderRadius:14,padding:"14px 16px"}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={()=>demander(p,"Demande de devis")} style={{flex:1,background:"transparent",border:`1px solid ${EV.or}`,borderRadius:9,padding:"9px",color:EV.or,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:SA}}>📋 Demander un devis</button>
                {!p.sur_devis && <button onClick={()=>demander(p,"Réservation")} style={{flex:1,background:EV.or,border:"none",borderRadius:9,padding:"9px",color:"#062b1d",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:SA}}>✓ Réserver</button>}
              </div>
            </div>
          ))}
        </div>
